...
impl Debug for FAT32Manager {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("FAT32Manager")
            ...
            .field("fat", &self.fat)
            .field("root_sec", &self.root_sec)
            .field("vroot_dirent", &self.vroot_dirent)
            .finish()
    }
}
...
impl FAT32Manager {
    ...
    /* 打开现有的FAT32  */
    // 接收一个实现了BlockDevice trait的Arc引用作为参数，并返回一个Arc<RwLock<Self>>实例，即FAT32Manager的多线程安全引用。
